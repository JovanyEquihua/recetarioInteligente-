      ...
      <div className="bg-white rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          {step === 2 && (
            <div className="animate-fade-in">
              <h3 className="text-lg font-semibold text-gray-800 mb-4">
                Elige hasta 3 tipos de comida preferidos
              </h3>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                {tiposComida.map((tipo) => (
                  <button
                    ...
                    disabled={
                      selectedTiposComida.length >= 3 &&
                      !selectedTiposComida.includes(tipo.id)
                    }
                    className={`p-4 rounded-lg border-2 flex flex-col items-center transition-all ${
                      selectedTiposComida.includes(tipo.id)
                        ? "border-[#8B1C62] bg-[#8B1C62]/10"
          ...
          {step === 3 && (
            <div className="animate-fade-in text-center py-8">
              ...
              <p className="text-gray-600 mb-6">
                Hemos guardado tus preferencias. Ahora podemos recomendarte
                recetas que se ajusten a tu gusto.
              </p>
              ...
              <div className="bg-gray-50 rounded-lg p-4 text-left max-w-md mx-auto">
                ...
                <p className="text-sm text-gray-600">
                  ...
                  {selectedSabores
                    .map((id) => sabores.find((s) => s.id === id).nombre)
                    .join(", ")}
                </p>
                <p className="text-sm text-gray-600 mt-1">
                  <span className="font-medium">Tipos de comida:</span>{" "}
                  {selectedTiposComida
                    .map((id) => tiposComida.find((t) => t.id === id).nombre)
                    .join(", ")}
                </p>
              </div>
            </div>
        ...
        <div className="border-t p-4 flex justify-between">
          <button
            ...
            className={`px-4 py-2 rounded-md ${
              step === 1
                ...
                : "text-[#8B1C62] hover:bg-[#8B1C62]/10"
            }`}
          >
          ...
          {step < 3 ? (
            <button
              ...
              className={`px-4 py-2 rounded-md ${
                ...
                (step === 2 && selectedTiposComida.length === 0)
                  ...
                  : "bg-[#8B1C62] text-white hover:bg-[#9e2a70]"
              }`}
            >
              Siguiente
            ...
            <button
              ...
              disabled={loading}
              className={`px-4 py-2 bg-[#8B1C62] text-white rounded-md hover:bg-[#9e2a70] ${
